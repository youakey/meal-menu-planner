  ...
  return (
    <Layout title="Кулинарная мастерская">
      ...
      <div className="mt-6 glass-card p-6 md:p-8">
        <div className="section-subtitle">Описание сервиса</div>
        <div className="section-title mt-1">О приложении</div>

        <div className="mt-4 text-sm leading-7 text-amber-100/70 md:text-base">
          <b className="text-amber-50">Планировщик меню и ингредиентов</b> — это современное
          веб-приложение для удобного ведения недельного меню, базы блюд и ингредиентов,
          с расчетом итогов и формированием корзины. Сайт работает через личный аккаунт:
          каждый пользователь может зарегистрироваться, подтвердить почту и хранить свои
          данные отдельно и безопасно. В системе можно создать базу ингредиентов с ценами
          и единицами измерения, собирать из них блюда, формировать несколько разных
          недельных меню под отдельные мероприятия или задачи, задавать вариации блюд,
          считать количество порций и автоматически получать итоговые расчеты как за всю
          неделю, так и за конкретный день. Дополнительно есть корзина, куда можно
          добавлять блюда из меню и отдельные ингредиенты, чтобы быстро посчитать нужный
          объем закупки. Преимущества сайта — удобный современный интерфейс, адаптация
          под компьютер и телефон, работа через личный кабинет, понятная структура учета,
          автоматизация ручных расчетов и возможность использовать систему как для личных
          нужд, так и для кухни, столовой, кафе, кейтеринга, лагеря или любого проекта,
          где нужно планировать питание и закупки.
        </div>
      </div>
