...
create table if not exists public.ingredient_products (
  ...
  package_unit text not null check (package_unit in ('kg', 'g', 'pcs', 'l')),
...
create table if not exists public.dish_ingredients (
  ...
  usage_unit text not null check (usage_unit in ('g', 'pcs', 'l')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.cart_items (
  ...
  portions numeric,
  quantity numeric,
  quantity_unit text check (quantity_unit in ('g', 'pcs', 'l')),
